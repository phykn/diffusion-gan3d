...
> **Local artifact provenance (checked 2026-08-09):** The figures and tables were regenerated from the EMA checkpoint `run/08070219/generator.pt` (SHA-256 `A5BA1D934305DAFC5141EDF0A61DAF97DB1D68EF94753575AF983CA6B6CFB208`). The controlled synthetic reference is the unconditioned seed-10000 sample in `scripts/gt_128.tiff`; evaluation volumes use separate seeds. The evaluation scripts write reference, metric, anchor-sweep, and overlap-ablation manifests under `temp/`. The checkpoint and manifests are local, Git-ignored artifacts, the Python dependencies are not version-pinned, and the evaluated run was initialized from a predecessor checkpoint that is no longer present. The current workspace therefore supports auditing the reported outputs, but not a clean end-to-end reproduction; the checkpoint, manifests, environment lock, and predecessor provenance must be archived before distribution.

> **Current-code status:** The training and inference implementation has changed since the reported checkpoint, including the schema-2 conditioning objectives, scale-consistency training, and endpoint-aware anchor-normal transition loss. These changes require a fresh run from initialization. The numerical results below describe the archived evaluated checkpoint and must be regenerated before they are attributed to the current implementation.
...
The evaluated checkpoint is the EMA output of a 20,000-step run with decay 0.999. That run loaded generator and critic weights from `run/08061756`, but the predecessor artifacts and training seed are absent; only the final weights, not optimizer state, were warm-started. The exact end-to-end training trajectory is therefore unavailable.
