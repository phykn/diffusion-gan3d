...
from src.train.config import DataConfig, load_config
...
class TrainConfigTest(unittest.TestCase):
    def test_relative_paths_resolve_from_the_config_directory(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            values["train"]["checkpoint"] = {
                ...
                "critic_0": "weights/critic_0.pt",
                "critic_1": None,
                "critic_2": None,
            }
            path = _write_config(root, values)
        ...
        self.assertEqual(config.optim.critic_local_weight, 0.5)
        self.assertEqual(
            config.train.checkpoint.model,
            (root / "config" / "weights" / "model.pt").resolve(),
        )
        self.assertEqual(
            config.train.checkpoint.critic_0,
            (root / "config" / "weights" / "critic_0.pt").resolve(),
        )
        self.assertIsNone(config.train.checkpoint.critic_1)
        self.assertIsNone(config.train.checkpoint.critic_2)

    def test_null_checkpoint_starts_every_model_from_scratch(self):
        values = _config_values()
        values["train"]["checkpoint"] = None
        with tempfile.TemporaryDirectory() as temp:
            config = load_config(_write_config(Path(temp), values))

        self.assertIsNone(config.train.checkpoint.model)
        self.assertIsNone(config.train.checkpoint.critic_0)
        self.assertIsNone(config.train.checkpoint.critic_1)
        self.assertIsNone(config.train.checkpoint.critic_2)

    def test_checkpoint_mapping_requires_all_four_files(self):
        values = _config_values()
        values["train"]["checkpoint"] = {"model": None}
        with (
            tempfile.TemporaryDirectory() as temp,
            self.assertRaisesRegex(
                ValueError,
                "model, critic_0, critic_1, and critic_2",
            ),
        ):
            load_config(_write_config(Path(temp), values))
    ...
    def test_anchor_section_is_required_and_validated_as_one_feature(self):
        values = _config_values()
        with tempfile.TemporaryDirectory() as temp:
            anchored = load_config(_write_config(Path(temp), values))
        ...
        for max_planes in (0, 4):
            values = _config_values()
            values["anchor"]["max_planes"] = max_planes
            with (
                self.subTest(max_planes=max_planes),
                tempfile.TemporaryDirectory() as temp,
                self.assertRaisesRegex(ValueError, "anchor.max_planes"),
            ):
                load_config(_write_config(Path(temp), values))
    ...
    def test_patch_divisibility_follows_the_configured_model_depth(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            shallow_config = load_config(_write_config(Path(temp), shallow))
        ...
        with tempfile.TemporaryDirectory() as temp:
            deep_config = load_config(_write_config(Path(temp), deep_valid))
    ...
    def test_local_critic_configuration_is_validated(self):
        cases = (
            (
                "weight",
                lambda values: values["optim"].update({"critic_local_weight": -0.1}),
                "optim.critic_local_weight",
            ),
            (
                "levels",
                lambda values: values["model"].update({"critic_channels": [4]}),
                "at least two levels",
            ),
        )
        for name, mutate, message in cases:
...
class DataLeakContractTest(unittest.TestCase):
    ...
    def test_phase_count_matches_uint8_storage(self):
        values = _config_values()
        values["data"]["num_phases"] = 257
        with tempfile.TemporaryDirectory() as temp:
            path = _write_config(Path(temp), values)
            with self.assertRaisesRegex(ValueError, "must not exceed 256"):
                load_config(path)

    def test_adam_beta2_must_be_less_than_one(self):
        values = _config_values()
        values["optim"]["beta2"] = 1.0
        with tempfile.TemporaryDirectory() as temp:
            path = _write_config(Path(temp), values)
            with self.assertRaisesRegex(ValueError, "beta2 < 1"):
                load_config(path)
