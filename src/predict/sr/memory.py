...
def estimate_sr_memory(
    ...
    *,
    ...
    shared = tiled or base_shape is not None
    ...
    return SRMemoryEstimate(
        ...
        coarse_bytes=coarse,
        state_bytes=4 * num_phases * math.prod(generation_shape) if shared else 0,
        ...
        * (
            interpolation
            + (4 * tile_voxels if shared else (tile_voxels if not labels else 0))
        )
        ...
        cpu_label_bytes=label_workspace if shared else 0,
        model_label_bytes=label_workspace if not shared else 0,
