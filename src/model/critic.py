...
class ConnectivityCritic2D(nn.Module):
    def __init__(
        self,
        num_phases: int,
        channels: Sequence[int],
        ...
        self.input = nn.Conv2d(3 * num_phases, widths[0], 3, padding=1)
        self.blocks = nn.ModuleList(
            AdaptiveResBlock2D(ch, ch, embedding_channels) for ch in widths
        )
        self.downsample = nn.ModuleList(
    ...
    def forward(
        ...
        return CriticScores(
            logits_global=(forward.logits_global + reverse.logits_global)
            * 0.5,
            logits_local=(forward.logits_local + reverse.logits_local)
            * 0.5,
        )
    ...
    def score_once(
        ...
        emb = self.axis_mlp(self.axis_embedding(axes)).to(dtype=triplets.dtype)
        x = self.input(triplets.flatten(1, 2))
        for idx, block in enumerate(self.blocks):
            x = self.run_block(block, x, emb)
            if idx == 1:
                local = F.silu(self.local_norm(x))
