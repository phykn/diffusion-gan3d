...
def main(argv: list[str] | None = None) -> None:
    ...
    source.add_argument("--input", type=Path, help="Existing LR label TIFF.")
    ...
    if args.weights:
        ...
        low = base.generate(domain=args.domain, seed=args.seed)
        del base
        if args.device == "cuda":
            torch.cuda.empty_cache()
    else:
        low = load_volume(args.input)
    output = (
        low
        if sr is None
        else sr.super_resolve(low, args.domain, args.seed, args.tile_size, args.overlap)
    )
    save_volume(output, args.output)
    if sr is not None and args.weights:
        save_volume(low, args.output.with_name(args.output.stem + "_lr.tiff"))
    metadata = {
